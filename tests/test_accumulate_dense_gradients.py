...
def test_multi_batch_matches_true_dataset_mean_with_uneven_final_batch():
    """batch_size=3 doesn't divide N=8 evenly (3, 3, 2). Naively summing
    per-batch means overweights the smaller final batch; the correct total
    must match a single direct backward() over the whole dataset.
    """
    mlp = Sequential(Linear(2, 4), ReLU(), Linear(4, 3))
    X = np.random.randn(8, 2)
    y = np.random.randint(0, 3, size=8)
    ...
    # old buggy "sum of per-batch means", kept to prove it differs from true mean
...
def test_skips_a_parameter_never_touched_this_sweep():
    """Same generic-helper concern as accumulate_gradients: a parameter the
    forward never touches leaves p.grad None and must be skipped, not crash.
    """
    ...
    extra_param = Tensor(np.ones((5, 5)), requires_grad=True)  # never used in forward
    model = _ModelWithDisconnectedParam(real_layer, extra_param)

    X = np.random.randn(10, 2)
    y = np.random.randint(0, 3, size=10)

    accumulate_dense_gradients(model, X, y, batch_size=4)  # uneven last batch, must not crash
...
def test_is_unmasked_unlike_weight_grad():
    """At a masked entry, weight.grad is exactly 0 (mask-gated), but the
    accumulated dense total isn't.
    """
    mlp = Sequential(Linear(2, 4), ReLU(), Linear(4, 3))
    X = np.random.randn(10, 2)
    y = np.random.randint(0, 3, size=10)
